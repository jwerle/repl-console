...
findCompletion = function(completions, line) {
  var found
  
  found = completions.filter(function(suspect) { return suspect.indexOf(line) == 0 })
  
  return [found.length ? found : completions, line]
}
    ...
    process.stdin.on('buffer', function(buffer, raw, log, filter){
      ...
      if (buffer) {
        ...
        if (buffer.match('\t') && (buffer = buffer.replace('\t', ''))) { // Enable a tab completion map
          ...
          completions = findCompletion(self.completions, suspect);
            
          if (completions.length === 2 && typeof completions[0] === 'object') {
            if (completions[0].length === 1) {
              found = completions[0][0];
